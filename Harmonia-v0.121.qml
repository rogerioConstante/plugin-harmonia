 ...
 for (var x=0;x<vozes.length;x++) {
  ...
  var myText = newElement(Element.STAFF_TEXT);
      if (!grau && grau !== 0) { myText.text = "?"; } else { myText.text = acordeGrau[grau].Grau;};
	   if (x > 0 && (vozes[x].posição[0] == vozes[x-1].posição[0])) { myText.pos.x = -3; } else {myText.pos.x = 0;};
      if (cursor.track > 3) { myText.pos.y = 13; } else { myText.pos.y = 28; };
      cursor.add(myText);
  };
  curScore.endCmd();
}
...
function segInvDissonancia() {
  ...
  for (var x=2;x<vozes.length;x++) { // percorre acordes
    ...
    var seguidos = false;
    var texto = "2ª inversão: tratamento do baixo";
